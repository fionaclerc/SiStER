% SiStER Initialize
% modified to keep track of topo markers on edge of cliff. (F. Clerc 2018)
% make sure to leave topo diffusion off, and BalanceSticky Layer off.
...
topo_x10 = linspace(0,GEOM(3).left,Ntopo/2); 
topo_y10 = GEOM(2).top+zeros([1 length(topo_x10)]);

topo_y20 = linspace(GEOM(2).top,GEOM(2).bot,Ntopo/2);
topo_x20 = GEOM(3).left+zeros([1 length(topo_y20)]);

topo_x1 = topo_x10;topo_x2 = topo_x20;
topo_y1 = topo_y10;topo_y2 = topo_y20;
topo_x = [topo_x1 topo_x2];topo_y = [topo_y1 topo_y2];

topo_marker_spacing1=mean(diff(topo_x1)); % initial mean spacing of topography markers

topo_marker_spacing2=mean(diff(topo_y2)); % initial mean spacing of topography markers

% initialize topo marker plastic strain (to zero) and strain rate (to one)
ep1=zeros(size(topo_x1));
epNH1=ep1;
epsIIm1=ones(size(topo_x1));
ep2=zeros(size(topo_x2));
epNH2=ep2;
epsIIm2=ones(size(topo_x2));
% initialize topo marker stresses
sxxm1=zeros(size(topo_x1));
sxym1=sxxm1;
sxxm2=zeros(size(topo_x2));
sxym2=sxxm2;
