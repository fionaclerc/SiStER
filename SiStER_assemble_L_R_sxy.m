 function [L, R, Kc, Kb]=SiStER_assemble_L_R_sxy(dx,dy,etas,etan,rho,BC,PARAMS,srhs_xx,srhs_xy)
% modified F. Clerc 2018, 2020. 
% allows for shear stresses as profile on bottom boundary.
% 
% feed in shear stress b.c. as BC.bottom = [4 0 0];
% and BC.bot_profile is profile of shear stress sig_xy.
% not implemented for remaining sides (or sigxx).
...
% 
        ...
        if (j==1 && i<=Ny-1 && i>=2) || (j==Nx-1 && i<=Ny-1 && i>=2) || (j==Nx) || (i==1 && j<=Nx-1) || (i==Ny && j<=Nx-1)
            ...
            if j==Nx-1 && i<=Ny-1 && i>=2  % right boundary without edges 
                ...
                elseif BC.right(1)==4 % prescribed stress
                    
                    % (i,j)
                    Lii(nn) = invy;
                    Ljj(nn) = invy;
                    Lvv(nn)  = (Kb./Kc).*etan(i,j+1)/(0.5*(dy(i)+dy(i-1)));% Zn is already multiplied through etan
                    nn = nn+1;
                    ...
                    R(invy,1) = (Kb./Kc).*(-BC.right(3) + ((1 - Zn(i,j+1)).*srhs_xx(i,j+1)));
